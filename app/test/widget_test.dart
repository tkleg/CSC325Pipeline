...
void main() {
  testWidgets("Tic Tac Toe Tests", (WidgetTester tester) async {
   ...
   // final allButtons = tester.widgetList(find.byType(ElevatedButton));
    ...
    //} else {
      ...
      //final firstButton = tester.widgetList(find.byType(ElevatedButton)).first;

      // Use the Finder returned by find.byWidget to create a Finder
      //final firstButtonFinder = find.byWidget(tester.widgetList(find.byType(ElevatedButton)).first);

      // Tap the first button using the Finder
      await tester.tap(find.byWidget(tester.widgetList(find.byType(ElevatedButton)).first));
      await tester.pump();
